...
exports.fblogin = async function (req, res) {
  ...
  try {
    ...
    if (items.length > 0) { //if user found then direct login into app
      response = await axios.post(`${MC_ENGINE_URL}/fblogin`, data);
      ...
      await axios.post(`${MC_ENGINE_URL}/registerUser`, userData);
      const loginResponse = await axios.post(`${MC_ENGINE_URL}/fblogin`, data);
